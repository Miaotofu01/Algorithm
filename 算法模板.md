...
ostream &operator<<(ostream &os, const pair<T1, T2> &p)//PII输入
{
    return os << "(" << p.first << ", " << p.second << ")";
}
template <typename T>
ostream &operator<<(ostream &os, const vector<T> &v)//vector输入
{
    os << "[";
...
#define debug(x) cerr << #x << "=" << x << endl;//可输出变量，一维vector
#define vdebug(x)         \
    cerr << #x << "=";    \
    for (auto p : x)      \
        cerr << p << " "; \
    cerr << endl; //可输出二维数组,PII一维数组,map
#define bin8(x) cerr << #x << "=" << bitset<8>(x) << endl;//输出八位二进制
#define rd read()
