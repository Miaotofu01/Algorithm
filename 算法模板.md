...
#define cerr  \
    if (true) \
    cerr // 调试输出开关 cerr不会被输出到答案中，但是会占用运行时间
const int N = 200, INF = 0x3f3f3f3f, MOD = 9998244353;
inline LL read()
{
    LL x = 0, y = 1;
    char c = getchar();
...
# 图论
## 二级结论
1. DAG中每个连通块一定有一个入度为0的起始点，通过这个点可以遍历到连通块中的所有点
2. 森林的边数=总边数-树的个数
3. 树上任意一点能达到最远的点一定是树直径的端点
4. 无向图的树中，叶子节点就是就是度数为1的点
## 常见子问题
### 判断图的连通性
常用于判断欧拉路径的存在
```cpp
...
int nbit = __lg(n);
vector<int> dep(n + 1);
vector<vector<int>> anc(n + 1, vector<int>(nbit + 1)), val(n + 1, vector<int>(nbit + 1));
function<void(int, int)> dfs = [&](int u, int f) mutable -> void
{
	for (auto [v, w] : g[u])
	{
...
for (int j = 1; j <= nbit; j++)
	...
	{
		anc[i][j] = anc[anc[i][j - 1]][j - 1];
		val[i][j] = min(val[i][j - 1], val[anc[i][j - 1]][j - 1]);//op
	}
// LCA
function<int(int, int)> LCA = [&](int u, int v) mutable -> int
{
	//如果维护森林的LCA，需要检查u和v是否在同一棵树上（并查集）
	int ans = INF;//op
	if (dep[u] < dep[v])
		swap(u, v);
	for (int i = nbit; i >= 0; i--)
	...
	for (int i = nbit; i >= 0; i--)
		if (anc[u][i] != anc[v][i])
		{
			ans = min(ans, min(val[u][i], val[v][i]));//op
			u = anc[u][i], v = anc[v][i];
		}
	return min(ans,min(val[u][0],val[v][0]));//op
};
```
#### tarjan算法（离线算法$O(n+m)$）
（暂空）
